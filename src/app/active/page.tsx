...
export default function ActiveJobs() {
  ...
  const acceptJob = async (requestId: string) => {
    ...
    try {
      await fetch(
        `https://toweasy-server.onrender.com/api/requests/${requestId}`,
        {
          method: "PATCH",
  ...
  return (
    <main className="container">
      ...
      {!isOnline ? (
        ...
        >
          <p>
            You are currently offline. Go to Dashboard to go online and see
            jobs.
          </p>
        </div>
      ) : (
            ...
            requests.map((req) => (
              <div key={req.id} className="job-card">
                <div className="job-header">
                  <div>
                    <strong>
                      {req.vehicleType} Recovery - {req.userName}
                    </strong>
                ...
                <div style={{ fontSize: "0.9rem" }}>
                  📍 Location: {req.location.latitude.toFixed(4)},{" "}
